...
###################################################################################################3
# Copy of file normfunctions.R
# Downloaded from sourceforge on August 23, 2018
...
input <- function(inputfile) {
   myY <<- read.csv(paste(inputfile, ".csv", sep=""));
   cohort <<- read.csv(paste(inputfile, ".cohorts.csv", sep=""));
   batch <<- read.csv(paste(inputfile, ".batches.csv", sep=""));

}

run <- function() {
cohort <<- factor(t(cohort));
batch <<- factor(t(batch));
YstdN <<- normalizeMixed(myY, cohort=cohort, batch=batch)
}

output <- function(outputfile) {
   write.table(YstdN, file=outputfile, sep=",");
}
